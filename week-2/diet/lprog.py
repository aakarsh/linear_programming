...
def _pivot_col(T, tol=1.0E-12, bland=False):
    ...
    if ma.count() == 0:
        if debug: print("=====================Finish Pivot Column:No Col======================")
        return False, np.nan
    if bland:
        if debug: print("=====================Finish Pivot Column:Bland======================")
        return True, np.where(ma.mask == False)[0][0]
    a,b= True, np.ma.where(ma == ma.min())[0][0]

    if debug:
        print("%5s:%s"%("pivot-column",b))
        print("=====================Finish Pivot Column======================")
...
def _pivot_row(T, pivcol, phase, tol=1.0E-12):
    ...
    # Array of constants greater in pivot column
    ma = np.ma.masked_where(T[:-k, pivcol] <= tol,
                            T[:-k, pivcol],copy=False)

    if ma.count() == 0: return (False, np.nan)

    # Array of all constants greater than than tolerance
    # masking constants where elements were masked

    mb = np.ma.masked_where(T[:-k, pivcol] <= tol,
                            T[:-k, -1],copy=False)

    q = mb / ma

    # return index of minimum element found
    # along with fact that it was found

    a,b = True, np.ma.where( q == q.min() )[0][0]

    if debug:
        print("%5s:%s"%("ma",ma))
        print("%5s:%s"%("mb",mb))
        print("%5s:%s"%("q",q))
        print("%5s:%s"%("pivot-row",b))
        print("=====================Finish Pivot Row======================")
...
def _solve_simplex(T, n, basis, maxiter=1000, phase=2, callback=None,
    ...
    # Have no idea why this is like this ??
    ...
    while not complete:
        ...
        if callback is not None:
            solution[:] = 0
            # do a direct assignment of constants from [0-m] in last column
            # to basis values  resot of the values will be zero.
...
def _linprog_simplex(c, A_ub=None, b_ub=None, A_eq=None, b_eq=None,
    ...
    T[-2, :n] = cc
    T[-2, -1] = f0
    # stop two rows short of last column
    b = T[:-2, -1]
    ...
    for i in range(m):
        if i < meq or b[i] < 0:
            ...
            avcount += 1
            if b[i] < 0: # make negative equation correspond to artificial variable
                b[i] *= -1
                T[i, :-1] *= -1
            # introduce the artificial variable at position basis[i]
            T[i, basis[i]] = 1
            T[-1, basis[i]] = 1
        else:
